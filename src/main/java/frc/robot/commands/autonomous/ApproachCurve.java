...
public class ApproachCurve extends Command {
	...
	/**
	 * Height of the left target, overall target, and right target (in that order as
	 * left as 0, and right as 2)
	 */
	private double[] target_height;

	/**
	 * Horizontal postion of the left target, overall target, and right target (in
	 * that order as left as 0, and right as 2)
	 */
	...
	/**
	 * Denotes if the drivers are in control of the robot during the duration of the
	 * command.
	 */
	...
	/**
	 * Keeps the id of the pipeline that is currently being asked to give values to
	 * the data arrays
	 */
	...
	protected void initialize() {
		...
		PIDapproach.setSetpoint(QuickAccessVars.SETPOINT_APPROACH); // Robot should aim to be be 50 in away from the
																	// target
		try {
	...
	private void auto() {
		...
		if (target_height[LEFT] == 0 || target_height[OVERALL] == 0 || target_height[RIGHT] == 0 || target_x[LEFT] == 0
			...
			return; // if any part of the data is unwritten to, then return out of function to avoid
					// the
	...
	private void drive() {
		...
		// Robot.camera.setPipeline(CameraSubsystem.PIPELINE_DRIVER); Conner does not
		// want this
	...
	/**
	 * Shift the setpoint of the {@link #PIDcenter} with the intent of making the
	 * robot turn to compensate the angle the robot is facing the target at. This
	 * should make the turn be an arc to make the robot perpendicular to the target.
	 */
	private void shiftCenter() {
		...
		// there is no log base in java so we must use the property log_b(a) =
		// log(a)/log(b)
		...
		// now with x being between -1 and 1 we can multiply by the degrees we want the
		// camera to turn
		// at most (25 degrees)
		double point = x * 25;
	...
	private void debug_missing() {
		if (target_height[LEFT] == 0) {
			System.out.println("WWDEBUG: Missing data: target_height[LEFT]");
		}
		if (target_height[OVERALL] == 0) {
			System.out.println("WWDEBUG: Missing data: target_height[OVERALL]");
		}
		if (target_height[RIGHT] == 0) {
			System.out.println("WWDEBUG: Missing data: target_height[RIGHT]");
		}
		if (target_x[LEFT] == 0) {
			System.out.println("WWDEBUG: Missing data: target_x[LEFT]");
		}
		if (target_x[OVERALL] == 0) {
			System.out.println("WWDEBUG: Missing data: target_x[OVERALL]");
		}
		if (target_x[RIGHT] == 0) {
			System.out.println("WWDEBUG: Missing data: target_x[RIGHT]");
		}
		if (target_distance == 0) {
			System.out.println("WWDEBUG: Missing data: target_distance");
		}
	}
