 ...
 */
@Deprecated
public class ApproachCurveVerbose extends Command {
	...
	*/
	...
	protected void initialize() {
		...
		System.out.println("WWDEBUG: initialize() of ApproachCurve");
	...
	protected void execute() {
		...
		{
			driver_control = true;
			System.out.println("WWDEBUG: driver_control = true");
		}
	...
	private void auto() {
		System.out.println("WWDEBUG: auto()");
		if (
		...
		{
			data_missing = true;
			System.out.println("WWDEBUG: Skipping moving, lacks data.");
		...
		if (Robot.camera.canSeeObject()) {
			...
			System.out.println("WWDEBUG: Can not see object, setting motion to 0");
		...
		System.out.println("WWDEBUG: -valueapproach : "+Double.toString(-valueapproach));
		System.out.println("WWDEBUG: -valuecenter : "+Double.toString(-valuecenter));
		Robot.drivetrain.arcadeDriveRaw(-valueapproach, -valuecenter);
		//System.out.println(valueapproach + " " + valuecenter);
	}

	private void drive() {
		System.out.println("WWDEBUG: drive()");
	...
	private void updateTargetData() {
		System.out.println("WWDEBUG: updateTargetData()");

		if (Robot.camera.getPipeline() == intendedPipe) { // wait for pipeline to be the intened one
			switch (intendedPipe) {
		
				case PIPELEFT: // Left
					System.out.println("WWDEBUG: Looking at Left");
				...
				case PIPEOVERALL: // Overall
					System.out.println("WWDEBUG: Looking at Both");
					...
					if ( !(1/1.1 < heightRatio && heightRatio < 1.1/1) || data_missing) {
						...
						heightRatio = 1;
						System.out.println("WWDEBUG: Staying looking for Both (assuming straight on)");
					}
					break;
				case PIPERIGHT: // Right
					System.out.println("WWDEBUG: Looking at Right");
		...
		try { // try statement makes the robot doesn't crash when target_height isn't set
			if (heightRatio != 1) {
				...
				System.out.println("WWDEBUG: Set heightRatio = "+Double.toString(heightRatio));
			}
		} catch(Exception e) {
			System.out.println(e);
			System.out.println("WWDEBUG: Error above forces heightRatio to be 0");
			heightRatio = 0;
		}
	...
	private void shiftCenter(){
		...
		// at most (25 degrees)
		double point = x * 25;
		System.out.println("WWDEBUG: Set PIDcenter's setpoint to: "+Double.toString(point));
